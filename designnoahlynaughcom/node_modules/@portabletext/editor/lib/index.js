...
const PortableTextEditorContext = createContext(null), usePortableTextEditor = () => {
  ...
  return editor;
}, debug$f = debugWithName("validate selection machine"), validateSelectionSetup = setup({
  types: {
    context: {},
    input: {},
    events: {}
  ...
  if (redos.length > 0) {
    ...
    if (debug$c("Redoing", step), step.operations.length > 0) {
      ...
      try {
        ...
        debug$c("Could not perform redo step", err), editor.remotePatches.splice(0, editor.remotePatches.length), Transforms.deselect(editor), editor.history = {
          undos: [],
          redos: []
  ...
  if (undos.length > 0) {
    ...
    if (debug$b("Undoing", step), step.operations.length > 0) {
      ...
      try {
        ...
        debug$b("Could not perform undo step", err), editor.remotePatches.splice(0, editor.remotePatches.length), Transforms.deselect(editor), editor.history = {
          undos: [],
          redos: []
...
function performEvent({
  ...
  mode === "send" && !isNativeBehaviorEvent(event) && (editor.undoStepId = defaultKeyGenerator()), debug$5(`(${mode}:${eventCategory(event)})`, JSON.stringify(event, null, 2));
